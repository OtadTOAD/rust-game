...
use nalgebra_glm::{TMat4, half_pi, identity, look_at, perspective, pi, vec3};
...
use crate::model::{DummyVertex, NormalVertex};
...
fn main() {
    let mut model = Model::new("assets/meshes/Monkey.glb").build();
    let mut vp = VP::new();
    vp.view = look_at(
        &vec3(0.0, 0.0, 0.1),
        &vec3(0.0, 0.0, 0.0),
    ...
    model.translate(vec3(0.0, 0.0, -5.0));
